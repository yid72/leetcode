...
/**
 * 1343. Maximum Product of Splitted Binary Tree -- Medium
 *
 ...
 */
public class Q01343_MaximumProductOfSplittedBinaryTree {
    private long maxProduct = 0;
    private Map<TreeNode, Long> sums = new HashMap<>();
    ...
    private long sum(TreeNode root) {
        ...
        if (!sums.containsKey(root)) {
            sums.put(root, sum(root.left) + sum(root.right) + root.val);
        }
        return sums.get(root);
    }

    private void maxProduct(TreeNode root, long total) {
        if (root == null) {
            return;
        }
        ...
        if (root.left != null) {
            ...
            maxProduct = Math.max(maxProduct, sumLeft * (total - sumLeft));
        }

        if (root.right != null) {
            long sumRight = sum(root.right);
            maxProduct = Math.max(maxProduct, sumRight * (total - sumRight));
        }
